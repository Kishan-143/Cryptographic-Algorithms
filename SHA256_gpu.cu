...
__device__ void sha256_transform(SHA256_CTX *ctx, const BYTE data[])
{
    ...
    // Here 64 iterations for SHA256, SHA224 we use 80 iterations.
    #pragma unroll 64
	for (i = 0; i < 64; ++i) {
...
JOB * JOB_init(BYTE * data, long size, char * fname) {
    ...
    cudaMallocManaged(&(j->data), size);
    j->data = data;
	j->size = size;
	for (int i = 0; i < 64; i++) j->digest[i]=0xff;
...
int main(int argc, char **argv) {
    ...
    if (a_file) {
		...
		for (n = 0; getline(&line, &len, f) != -1; n++){}
  	    cudaMallocManaged(&jobs, (n+1) * sizeof(JOB *));
        fseek(f, 0, SEEK_SET);
		...
		while(read!=-1){
            ...
            if(read==-1) continue;
            BYTE * buffer;
            cudaMallocManaged(&buffer, (read-1)*sizeof(BYTE));
    ...
    cout << "Avrage total time : "<< chrono::duration_cast<chrono::nanoseconds>((end2 - start2)/n).count()<< " ns" << endl;
    ...
    FILE * fp2;
	FILE * fp3;
    fp2=fopen("result_gpu.txt","w");
    fp3=fopen("result_gpu_detail.txt","w");
